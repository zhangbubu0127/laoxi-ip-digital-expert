...
_PLANNER_SYSTEM = (
    ...
    "- 老板报视频/文章数据（浏览/点赞/评论/收藏/转发/播放/留资 带数字）→ 动作=数据回流，params 放 date/topic/data；"
    "原话里没日期就让系统按已发回落，date 可省。\n"
    ...
    "- 老板直接 @某专家 闲聊 → 动作=专家闲聊。\n"
    "- 老板要调研/查证市场信息（「调研一下X」「帮我查查X怎么样」「了解下X的最新情况」「X怎么做的」）"
    "→ 动作=调研，task 写清调研问题，派席小题用实时搜索调研后带来源回答。\n"
    "- 老板要看表格/排期（「看一下表格/看下排期/看看排期表/排期表链接发我」）→ 动作=看排期表（立即给链接），别判成别的。\n"
    "- 动作=确认已发布 必须在原话里有「已发布/发了/发过了」等确认信号；"
    "光说「看一下表格/状态」「把状态调一下」不构成标已发，别判。\n"
    ...
    "3. params 只放：count（数量）、topic（角度/主题/选题）、content_type（曝光/留资/信任）、date（日期）、data（数据回流的数据串）；"
    "没有就留 {}。确认已发布/数据回流要把能从原话认出的 date/topic/data 填进 params。\n"
